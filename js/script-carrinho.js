...
function armazenarValores(item){
    ...
    InserirProduto(objProduto,arrayProdutos,cardAtual);
    listarItems();
    calcularTotal();
}

function InserirProduto(objProduto,array)
{

    const existeNoArray = array.find((produto) => objProduto.nome === produto.nome);
    const i = array.findIndex((produto) => objProduto.nome === produto.nome);

    if(existeNoArray){
        objProduto.quantidade >= 1 ? arrayProdutos[i] = objProduto : alert("Adicione ao menos uma quantidade do produto para continuar");}
    else{
        objProduto.quantidade >= 1 ? array.push(objProduto) : alert("Adicione ao menos uma quantiade do produto");
        ...
        divCarrinho.innerHTML += `<div class="col-3">
            <div class="card">
                <div class="card-body">
                  ...
                  <p class="card-text">Valor Total: ${produto.valorTotal}</p>
